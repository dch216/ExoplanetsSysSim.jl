## ExoplanetsSysSim/examples/dr25_gaia_m/christiansen_func.jl
## (c) 2019 Danley C. Hsu & Eric B. Ford
# Collection of functions specific to estimating DR25 M
...
function generate_num_planets_m_fgk_ratio(s::Star, sim_param::SimParam)
  ...
  const p_dim = length(limitP)-1
  const r_dim = length(get_any(sim_param, "r_lim_full", Array{Float64,1}))-1
  sum_lambda = 0
  for i in 1:p_dim
